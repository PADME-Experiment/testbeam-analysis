...
void Scan(int kind=1, bool SaveImages=false, int max_mult=0){
  ...
  */
  ...
  for(int irun=0;irun<3;irun++){
    ...
    if(irun == 0){run = 470; beam_energy_MeV = 450; detector = "PMT";beam_spread=1.1;}
    else if(irun == 1){run = 471; beam_energy_MeV = 250; detector = "PMT";beam_spread=2.4;}
    else if(irun == 2){run = 472; beam_energy_MeV = 250; detector = "PMT";beam_spread=2.4;}
    ...
    */
    ...
    int n_peaks = spec->Search(H2,5.,"",0.0002);
    //int n_peaks = spec->SearchHighRes(source,dest,nbins,100,0.001,kFALSE,3,kTRUE,10);
    ...
    */
    ...
    if(max_mult==0 && run==470) n_max=2;
    if(max_mult==0 && run==471) n_max=4;
    if(max_mult==0 && run==472) n_max=3;
