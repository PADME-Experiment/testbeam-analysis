...
Analyse::FillOscCumulatives(){
  ...
  if(fSignalChannels[Universe::eROCh::eCalo]->IsSinglePartBeam()){
    nfilled++;
  }
...
#pragma omp single nowait
    {
      ...
      for(UShort_t s=0;s<1024;s++){
        fHists.hist2f_OscCum[chan_i]->Fill(s,ch.GetSamp(s));
        if(fSignalChannels[Universe::eROCh::eCalo]->IsSinglePartBeam()){
          ...
          if(nfilled<100){
            ...
            fHists.hist2f_SignalStudyLowPass[chan_i]->SetBinContent(s+1,nfilled,-ch.GetValT0PedLowPass(s));
          }
        }
      }
    }
...
#pragma omp single nowait
    {
      ...
      for(auto sigjj_it=std::next(sigii_it);sigjj_it!=fSignalChannels.end();++sigjj_it){
        ...
        if(true
            ...
            &&int(chii.GetPhEAbs())+5>int(chjj.GetPhEAbs()) //comparable charge for ii and jj
...
#pragma omp single nowait
    {
      ...
      ch.FFTLowPass(fNFreqToCancelFFT);
      ch.CalcTimeCharge();
    }
  }
}
