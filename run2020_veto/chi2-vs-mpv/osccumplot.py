...
def osc_cum_cap(name,inroot,ax0, ax1=None):
...
def osc_cum(name,inroot,ax0, ax1=None):
    ...
    power=.29
    ax0.pcolorfast(xarr,yarr,arr**(power), cmap='rainbow')
    ax0.set_xlabel('Cell position')
    ax0.set_ylabel('Amplitude[ADU]')
    ax0.set_ylim(3576, 3604)
    ax0.set_xticks(ticks)
    #ax0.set_xticklabels((ticks/5).astype(int))
    if ax1:
...
filename = 'chi2-10000evt.root'
ifile_chi2 = root.TFile(filename)

filename = 'mpv-10000evts.root'
ifile_mpv = root.TFile(filename)

def chi2_vs_mpv():
    fig, ax = plt.subplots(2,2, sharey=True, squeeze=True, figsize=(8.5,5), gridspec_kw={'width_ratios': [5, 1]})
    osc_cum('b25_Pedestal_cal_Cell_ch6',ifile_mpv ,*ax[0])
    osc_cum('b25_Pedestal_cal_Cell_ch6',ifile_chi2,*ax[1])
    ax[0][1].set_xticklabels('')
    ax[0][0].set_xticklabels('')
    fig.text(1,.75, '\nMPV calibration'     , rotation='vertical', in_layout=False, ha='right', va='center')
    fig.text(1,.32, '\n$\chi^2$ calibration', rotation='vertical', in_layout=False, ha='right', va='center')
    fig.tight_layout()
    fig.subplots_adjust(hspace=0.03, wspace=0.03)
    fig.savefig('chi2-vs-mpv.pdf')


#fig, ax = plt.subplots(figsize=(8.5,5))
fig, ax = plt.subplots(1,2, sharey=True, squeeze=True, figsize=(8.5,5), gridspec_kw={'width_ratios': [5, 1]})
osc_cum_cap('b25_ch3_Cells_time_cap_9_cap_9'        ,ifile_mpv,*ax)
fig.tight_layout()
fig.savefig('cap9-timedep.pdf')

exit()

#fig, ax = plt.subplots(figsize=(8.5,5))
fig, ax = plt.subplots(1,2, sharey=True, squeeze=True, figsize=(8.5,5), gridspec_kw={'width_ratios': [5, 1]})
osc_cum_cap('b25_PedestalNoCellPedCor_ch3',ifile,*ax)
